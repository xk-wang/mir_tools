...
def note_search(data, file_path):
    ...
    for idx in valid_indexs[1:]:
        if idx - last > 1:
            onset_max_idx = np.argmax(
                onset_prob[start:last+1, 0], axis=0) + start
            pitch_max_idxs = np.argmax(
                pitch_prob[start:last+1, :], axis=0) + start

            for i in range(88):
                pitch_max_idx = pitch_max_idxs[i]
                if pitch_prob[pitch_max_idx, i] > pitch_thrd:
                    buffer_notes.append(
                        [(pitch_max_idx+onset_max_idx)/2*hop_len/sr, i+21])
...
def raw_res(args):
    ...
    for filename in filenames:
        ...
        onset_data = np.loadtxt(os.path.join(
            args.raw_onset_path, filename_basename.replace('.pitch_raw', '.onset_raw')))
        ...
        file_path = os.path.join(
            result_dir, filename_basename.replace('.pitch_raw', '.res'))
